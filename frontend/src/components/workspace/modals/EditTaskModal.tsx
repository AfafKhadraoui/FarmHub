...
export function EditTaskModal({
  ...
  useEffect(() => {
    if (isOpen && taskData) {
      ...
      const dueDateTime = taskData.dueDate
        ? new Date(taskData.dueDate)
        : new Date();
      const dateStr = dueDateTime.toISOString().split("T")[0];
      const timeStr = dueDateTime.toTimeString().slice(0, 5);
      ...
      setFormData({
        ...
        dueDate: dateStr,
        dueTime: timeStr,
        workers: taskData.taskAssignments?.map((a: any) => a.workerId) || [],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      (window as any).showToast?.("Task updated successfully!", "success");
      onClose();
    } catch (error) {
            ...
            <div>
              ...
              <input
                ...
                className={`w-full h-11 px-4 border ${
                  errors.dueDate ? "border-red-500" : "border-[#D1D5DB]"
